...
func setupLocal(t *testing.T) (*Local, string, func()) {
	...
	l, err := NewLocal(dir, 12) //returns connection instance
	if err != nil {
		t.Fatal(err)
	}
...
func TestSavesContentsofReader(t *testing.T) {
	...
	defer cleanup() //called the cleanup function as returned function

	// save a file
	err := l.Save(savePath, bytes.NewBuffer([]byte(fileContents)))
	assert.NoError(t, err)

	// chech the file has been correctly written
