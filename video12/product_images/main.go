...
func main() {
	...
	gh.Handle(
		"/images/{id:[0-9]+}/{filename:[a-zA-Z]+\\.[a-z]{3}}", //regex path to get the image
		// have to apply gzip here
		http.StripPrefix("/images/", http.FileServer(http.Dir(*basePath))), // store what we got in basepath "file.png"
	)
	gh.Use(mw.GzipMiddleware) // applied middleware
